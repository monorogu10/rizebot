const SAVE_CHANNEL_ID = '1412649363037229176';
const STORAGE_FILE_NAME = 'leaderboard-data.json';
const REGISTER_STORAGE_FILE_NAME = 'register-interview-data.json';
const INTERVIEW_TRANSCRIPT_INDEX_FILE_NAME = 'interview-transcripts-index.json';
const INTERVIEW_TRANSCRIPT_FILE_PREFIX = 'interview-transcripts';
const SUBMISSION_STORAGE_FILE_NAME = 'submission-data.json';
const MODERATION_STORAGE_FILE_NAME = 'moderation-data.json';
const REGISTER_ROLE_ID = '1457175840386191548';
const LEGACY_ROLE_ID = '1455124395470094416';
const MINECRAFT_REGISTER_ROLE_ID = '1524356030241312838';
const MINECRAFT_REGISTER_PENDING_ROLE_ID = '1524350331654504592';
const MINECRAFT_REGISTER_REJECTED_ROLE_ID = '1524350558922870814';
const MINECRAFT_REGISTER_RESET_ADMIN_ID = '683306687511265290';
const TOPUP_ADMIN_DISCORD_ID = '683306687511265290';
const TOPUP_BRIDGE_HOST = process.env.TOPUP_BRIDGE_HOST || '0.0.0.0';
const parsedTopupBridgePort = Number(process.env.TOPUP_BRIDGE_PORT);
const TOPUP_BRIDGE_PORT = Number.isInteger(parsedTopupBridgePort) && parsedTopupBridgePort > 0
  ? parsedTopupBridgePort
  : 8787;
const TOPUP_BRIDGE_TOKEN = process.env.TOPUP_BRIDGE_TOKEN || 'srx:v1:discord-access:9nK4pZ2q';
const MINECRAFT_CHAT_LOG_CHANNEL_ID = process.env.MINECRAFT_CHAT_LOG_CHANNEL_ID || '1519260589208240219';
const MINECRAFT_AUDIT_LOG_CHANNEL_ID = process.env.MINECRAFT_AUDIT_LOG_CHANNEL_ID || MINECRAFT_CHAT_LOG_CHANNEL_ID;
const MINECRAFT_ERROR_LOG_CHANNEL_ID = process.env.MINECRAFT_ERROR_LOG_CHANNEL_ID || MINECRAFT_AUDIT_LOG_CHANNEL_ID;
const MINECRAFT_ORGANIZATION_LOG_CHANNEL_ID = process.env.MINECRAFT_ORGANIZATION_LOG_CHANNEL_ID || MINECRAFT_AUDIT_LOG_CHANNEL_ID;
const SOCIABUZZ_TOPUP_LOG_CHANNEL_ID = process.env.SOCIABUZZ_TOPUP_LOG_CHANNEL_ID || '1519567676836413490';
const SOCIABUZZ_SOURCE_CHANNEL_IDS = new Set(
  String(process.env.SOCIABUZZ_SOURCE_CHANNEL_IDS || '1195891719154184295')
    .split(',')
    .map(item => item.trim())
    .filter(Boolean)
);
const SOCIABUZZ_WEBHOOK_TOKEN = process.env.SOCIABUZZ_WEBHOOK_TOKEN || '';
const MINECRAFT_INFO_CHANNEL_ID = '1412304090146541748';
const MINECRAFT_INFO_URL = 'https://discord.com/channels/1195884175442587779/1412304090146541748';
const PRIVATE_CHAT_CHANNEL_ID = '1465702373946163353';
const SERVER_STATUS_CHANNEL_ID = process.env.SERVER_STATUS_CHANNEL_ID || '1465702373946163353';
const SUBMISSION_CHANNEL_ID = '1466590962066133023';
const SUBMISSION_ROLE_ID = '1195887506021957697';
const RATING_PREFIX = '[rate]';
const RATING_APPROVE_EMOJI = '\u2705';
const RATING_REJECT_EMOJI = '\u274C';
const RATING_FIRE_EMOJI = '\uD83D\uDD25';
const RATING_SKULL_EMOJI = '\uD83D\uDC80';
const RATING_NEUTRAL_EMOJI = '\uD83D\uDDFF';
const RATING_MIN_APPROVALS = 11;
const SUBMISSION_SCAN_LIMIT = 120;
const SUBMISSION_SCAN_MAX_AGE_DAYS = 30;
const SUBMISSION_SCAN_DELAY_MS = 350;
const TRASH_EMOJI = '\uD83D\uDDD1\uFE0F';
const TRASH_MIN_COUNT = 5;
const PETITION_VOTE_EMOJI = '\u2705';
const PETITION_MIN_VOTES = 17;
const PETITION_WINDOW_MS = 60 * 60 * 1000;
const TIMEOUT_DURATION_MS = 24 * 60 * 60 * 1000;
const REGISTRATION_INBOX_CHANNEL_ID = '1209816094156529745';
const INTERVIEW_CATEGORY_ID = process.env.INTERVIEW_CATEGORY_ID || '1524353368418553866';
const INTERVIEW_ARCHIVE_CATEGORY_ID = process.env.INTERVIEW_ARCHIVE_CATEGORY_ID || '';
const INTERVIEW_ARCHIVE_CATEGORY_NAME = process.env.INTERVIEW_ARCHIVE_CATEGORY_NAME || '';
const INTERVIEW_ADMIN_ROLE_IDS = new Set(
  ['1351714844134604931', ...String(process.env.INTERVIEW_ADMIN_ROLE_IDS || '').split(',')]
    .map(item => String(item).trim())
    .filter(Boolean)
);
const LAW_ADMIN_ROLE_IDS = new Set(
  String(process.env.LAW_ADMIN_ROLE_IDS || '')
    .split(',')
    .map(item => item.trim())
    .filter(Boolean)
);
const WELCOME_CHANNEL_ID = '1195884175912358031';
const LEAVE_CHANNEL_ID = '1412648951638917271';
const BOT_OUTPUT_CHANNEL_IDS = new Set([
  '1467759150346928170',
  '1209816094156529745',
  '1411683951575040160',
  '1465702373946163353',
  MINECRAFT_CHAT_LOG_CHANNEL_ID,
  MINECRAFT_AUDIT_LOG_CHANNEL_ID,
  MINECRAFT_ERROR_LOG_CHANNEL_ID,
  MINECRAFT_ORGANIZATION_LOG_CHANNEL_ID,
  SOCIABUZZ_TOPUP_LOG_CHANNEL_ID,
  '1517517579587948645',
  '1265850087288868864',
  '1259789142720647180'
]);

const LINK_REGEX = /https?:\/\/\S+/i;
const ALLOWED_GIF_HOSTS = new Set([
  'tenor.com',
  'media.tenor.com',
  'giphy.com',
  'media.giphy.com',
  'i.giphy.com',
  'i.imgur.com',
  'cdn.discordapp.com'
]);
const KEYWORD_LINKS = {
  monodeco: 'https://monodeco.my.id/',
  mdco: 'https://monodeco.my.id/',
  mde: 'https://monodeco.my.id/',
  engine: 'https://monodeco.my.id/',
  mdengine: 'https://monodeco.my.id/',
  monodecoengine: 'https://monodeco.my.id/',
  monodecogen4: 'https://monodeco.my.id/',
  decorize: 'https://monodeco.my.id/',
  gen4: 'https://monodeco.my.id/',
  mdg4: 'https://monodeco.my.id/',
  server: 'https://discord.gg/XvMgRJpRBV',
  live: 'https://www.youtube.com/@monodeco_md',
  update: 'https://www.youtube.com/@monodeco_md',
  liveupdate: 'https://www.youtube.com/@monodeco_md',
  donate: 'https://sociabuzz.com/monodeco/support',
  donasi: 'https://sociabuzz.com/monodeco/support',
  support: 'https://sociabuzz.com/monodeco/support',
  website: 'https://monodeco.my.id/'
};
const COMMAND_CHANNELS = new Set(BOT_OUTPUT_CHANNEL_IDS);

module.exports = {
  SAVE_CHANNEL_ID,
  STORAGE_FILE_NAME,
  REGISTER_STORAGE_FILE_NAME,
  INTERVIEW_TRANSCRIPT_INDEX_FILE_NAME,
  INTERVIEW_TRANSCRIPT_FILE_PREFIX,
  SUBMISSION_STORAGE_FILE_NAME,
  MODERATION_STORAGE_FILE_NAME,
  REGISTER_ROLE_ID,
  LEGACY_ROLE_ID,
  MINECRAFT_REGISTER_ROLE_ID,
  MINECRAFT_REGISTER_PENDING_ROLE_ID,
  MINECRAFT_REGISTER_REJECTED_ROLE_ID,
  MINECRAFT_REGISTER_RESET_ADMIN_ID,
  TOPUP_ADMIN_DISCORD_ID,
  TOPUP_BRIDGE_HOST,
  TOPUP_BRIDGE_PORT,
  TOPUP_BRIDGE_TOKEN,
  MINECRAFT_CHAT_LOG_CHANNEL_ID,
  MINECRAFT_AUDIT_LOG_CHANNEL_ID,
  MINECRAFT_ERROR_LOG_CHANNEL_ID,
  MINECRAFT_ORGANIZATION_LOG_CHANNEL_ID,
  SOCIABUZZ_TOPUP_LOG_CHANNEL_ID,
  SOCIABUZZ_SOURCE_CHANNEL_IDS,
  SOCIABUZZ_WEBHOOK_TOKEN,
  MINECRAFT_INFO_CHANNEL_ID,
  MINECRAFT_INFO_URL,
  PRIVATE_CHAT_CHANNEL_ID,
  SERVER_STATUS_CHANNEL_ID,
  SUBMISSION_CHANNEL_ID,
  SUBMISSION_ROLE_ID,
  RATING_PREFIX,
  RATING_APPROVE_EMOJI,
  RATING_REJECT_EMOJI,
  RATING_FIRE_EMOJI,
  RATING_SKULL_EMOJI,
  RATING_NEUTRAL_EMOJI,
  RATING_MIN_APPROVALS,
  SUBMISSION_SCAN_LIMIT,
  SUBMISSION_SCAN_MAX_AGE_DAYS,
  SUBMISSION_SCAN_DELAY_MS,
  TRASH_EMOJI,
  TRASH_MIN_COUNT,
  PETITION_VOTE_EMOJI,
  PETITION_MIN_VOTES,
  PETITION_WINDOW_MS,
  TIMEOUT_DURATION_MS,
  REGISTRATION_INBOX_CHANNEL_ID,
  INTERVIEW_CATEGORY_ID,
  INTERVIEW_ARCHIVE_CATEGORY_ID,
  INTERVIEW_ARCHIVE_CATEGORY_NAME,
  INTERVIEW_ADMIN_ROLE_IDS,
  LAW_ADMIN_ROLE_IDS,
  WELCOME_CHANNEL_ID,
  LEAVE_CHANNEL_ID,
  BOT_OUTPUT_CHANNEL_IDS,
  LINK_REGEX,
  ALLOWED_GIF_HOSTS,
  KEYWORD_LINKS,
  COMMAND_CHANNELS
};
